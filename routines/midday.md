...
- tail of memory/TRADE-LOG.md (entries, thesis, stops per position)
...
STEP 5 — Thesis check. If a thesis broke intraday (catalyst invalid, sector
rolling over, bad news), cut the position even if not at -7% yet.
Document reasoning in TRADE-LOG.

STEP 6 — Optional intraday research via the WebSearch tool if something is moving
sharply with no obvious cause. Append afternoon addendum to RESEARCH-LOG.

STEP 7 — Notification: only if action was taken.
  bash scripts/telegram.sh "<action summary: what was cut/tightened and why>"
